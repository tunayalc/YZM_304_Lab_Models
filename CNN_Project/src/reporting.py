...
def plot_class_distribution(class_distribution: Dict[str, int], output_path: Path) -> None:
    ...
    ax.set_title("Egitim Sinif Dagilimi")
    ax.set_xlabel("Sinif")
...
def plot_training_curves(histories: Dict[str, List[Dict[str, float]]], output_path: Path) -> None:
    ...
    axes[0].set_title("Egitim ve Dogrulama Kayip Egrileri")
    axes[0].set_xlabel("Epoch")
    axes[0].set_ylabel("Loss")
    axes[1].set_title("Egitim ve Dogrulama Dogruluk Egrileri")
...
def plot_metric_comparison(results: Iterable[Dict[str, object]], output_path: Path) -> None:
    ...
    ax.set_title("Bes Model Icin Accuracy ve F1 Macro Karsilastirmasi")
    ax.set_xlabel("Model")
    ax.set_ylabel("Skor")
